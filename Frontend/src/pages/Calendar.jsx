...
const CalendarPage = () => {
  ...
  const initialDate = (() => {
    ...
    if (y && m !== null) return new Date(Number(y), Number(m), 1);
    return new Date();
  })();
                ...
                {cells.map((day, i) => {
                  ...
                  return (
                    <button
                      key={day}
                      onClick={() =>
                        hasExpenses && setSelectedDay(isSelected ? null : day)
                      }
                      ...
                      className={`aspect-square rounded-lg border flex flex-col items-center justify-center gap-0.5 transition-all ${
                        ...
                        hasExpenses
                          ? "cursor-pointer hover:border-indigo-400/50"
                          : "cursor-default"
                      }`}
                    >
                      <span
                        className={`text-xs sm:text-sm ${isToday ? "text-indigo-300 font-bold" : "text-gray-300"}`}
                      >
                        {day}
                      </span>
            ...
            <div className="mt-5 bg-[#111118] border border-indigo-500/30 rounded-2xl p-5 animate-slide-up">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold flex items-center gap-2">
                  ...
                  {dayTotals[selectedDay].toLocaleString()}
                </h3>
                <button
